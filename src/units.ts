// Unit conversion + formatting. Internally everything is SI (m, m/s, seconds);
// these helpers turn SI values into display strings for the chosen system.
...
const MS_TO_FPM = 196.8503937; // m/s → feet per minute
...
export interface UnitFormatter {
  system: UnitSystem;
  /** Distance, choosing km/mi. `digits` overrides default precision. */
  distance(meters: number, digits?: number): string;
  /** Short distance (radius etc.) in m/ft. */
  smallDistance(meters: number, digits?: number): string;
  /** Altitude in m/ft. */
  altitude(meters: number): string;
  /** Horizontal speed in km/h / mph. */
  speed(ms: number, digits?: number): string;
  /** Vertical speed in m/s (metric) or ft/min (imperial). */
  vario(ms: number): string;
  /** Bare unit labels, for axis titles etc. */
  labels: {
    distance: string;
...
export function makeFormatter(system: UnitSystem): UnitFormatter {
  ...
  return {
    ...
    speed: (ms, digits = 1) =>
      `${fmt(ms * (metric ? MS_TO_KMH : MS_TO_MPH), digits)} ${
        metric ? "km/h" : "mph"
      }`,
    vario: (ms) =>
      metric
