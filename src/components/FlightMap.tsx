...
import { buildVarioSegments, PHASE_COLORS, VARIO_LEGEND } from "../colors";

interface Props {
  flight: Flight;
  /** Phase to outline (hover or selection). */
  highlight: Phase | null;
  /** Phase to zoom to (click selection only). */
  zoomTo: Phase | null;
        ...
        {highlightPositions && (
          <>
            ...
            <Polyline
              positions={highlightPositions}
              pathOptions={{
                color: PHASE_COLORS[highlight!.kind],
                weight: 3.5,
                opacity: 1,
              }}
            />
...
/** Fit the map to the whole track whenever the flight changes. */
...
/** Zoom to the selected phase when it changes. */
