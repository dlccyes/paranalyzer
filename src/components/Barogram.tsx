...
import { PHASE_COLORS, varioColor } from "../colors";
import { formatClock, type UnitFormatter } from "../units";

interface Props {
  flight: Flight;
  fmt: UnitFormatter;
  /** Phase to emphasise (hover or selection). */
  active: Phase | null;
...
export function Barogram({
  ...
  const model = useMemo(() => {
    ...
    // Vario-coloured polyline runs (merge consecutive same-colour fixes).
    ...
    for (let k = 0; k <= tickCount; k++) {
      const a = aMin + ((aMax - aMin) * k) / tickCount;
  ...
  const idxFromClientX = (clientX: number): number | null => {
    ...
    if (lo > s && Math.abs(derived.t[lo - 1] - t) < Math.abs(derived.t[lo] - t)) {
      return lo - 1;
    }
    return lo;
  };
        ...
        >
          ...
          {phases.map((p, i) => (
            <rect
              ...
              width={Math.max(
                1,
                model.xs(derived.t[p.endIdx]) - model.xs(derived.t[p.startIdx]),
              )}
              height={6}
          ...
          {active && (
            <rect
              ...
              width={Math.max(
                1,
                model.xs(derived.t[active.endIdx]) -
                  model.xs(derived.t[active.startIdx]),
              )}
              height={PLOT_H}
          ...
          {model.yticks.map((t, i) => (
            <g key={i}>
              <line
                ...
                className="grid-line"
              />
              <text x={PAD.l - 6} y={t.y + 3} className="axis-label y">
                {t.label}
              </text>
            </g>
          ))}
          ...
          {model.xticks.map((t, i) => (
            <text key={i} x={t.x} y={H - 8} className="axis-label x">
              {t.label}
            </text>
          ))}
        ...
        {hover && (
          <div
            ...
            style={{
              left: `${(hoverX / W) * 100}%`,
              transform:
                hoverX > W * 0.7 ? "translateX(-105%)" : "translateX(8px)",
            }}
          >
            <div>
              <b>{fmt.altitude(hover.fix.alt)}</b>
            </div>
